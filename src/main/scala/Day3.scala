...
object Day3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val closestDistanceIntersection =
      findClosestDistanceIntersection(wire1, wire2)

    println(s"closest distance intersection [$closestDistanceIntersection]")

    val closestStepsIntersection = findLeastStepsIntersection(wire1, wire2)

    println(s"closest steps intersection [$closestStepsIntersection]")
  }

  sealed abstract class WireDirection(val value: String,
                                      val action: (Int, Int) => (Int, Int))
      extends StringEnumEntry

  object WireDirections extends StringEnum[WireDirection] {

    case object Up extends WireDirection("U", (x, y) => (x, y + 1))
    case object Down extends WireDirection("D", (x, y) => (x, y - 1))
    case object Left extends WireDirection("L", (x, y) => (x - 1, y))
    case object Right extends WireDirection("R", (x, y) => (x + 1, y))

    val values = findValues

  }

  case class WireInstruction(direction: WireDirection, length: Int)

  def parseInput(input: String): Option[WireInstruction] = {
    val (direction, length) = input.splitAt(1)
    (WireDirections.withValueOpt(direction), length.toIntOption)
      .mapN(WireInstruction)
  }

  def generateWireSegment(
    previousSegment: ((Int, Int), NonEmptyList[Int]),
    wireInstruction: WireInstruction
  ): List[((Int, Int), NonEmptyList[Int])] = {
    (0 until wireInstruction.length)
      .foldLeft(List.empty[((Int, Int), NonEmptyList[Int])])((segments, _) => {
        val (coords, position) = segments.lastOption.getOrElse(previousSegment)

        segments :+ wireInstruction.direction.action
          .tupled(coords) -> position.map(_ + 1)
      })
  }

  def generateWireMap(
    wireInstructions: List[WireInstruction]
  ): Map[(Int, Int), NonEmptyList[Int]] = {
    wireInstructions
      .foldLeft(List.empty[((Int, Int), NonEmptyList[Int])])(
        (wires, instruction) =>
          wires ++ generateWireSegment(
            wires.lastOption.getOrElse(((0, 0), NonEmptyList.one(0))),
            instruction
        )
      )
      .toMap
  ...
  def findIntersections(
    wireMap: Map[(Int, Int), NonEmptyList[Int]]
  ): Map[(Int, Int), NonEmptyList[Int]] = {
    wireMap.filter(_._2.size > 1)
  }

  def findClosestDistanceIntersection(wire1: List[WireInstruction],
                                      wire2: List[WireInstruction]): Int =
    findIntersections(generateWireMap(wire1) |+| generateWireMap(wire2)).keys
      .map(coords => coords._1.abs + coords._2.abs)
      .min

  def findLeastStepsIntersection(wire1: List[WireInstruction],
                                 wire2: List[WireInstruction]): Int =
    findIntersections(generateWireMap(wire1) |+| generateWireMap(wire2)).values
      .map(_.toList.sum)
      .min
}
